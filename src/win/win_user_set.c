...
void UnloadUserSetBmp() {
	UnloadBgBmp();
	UnloadWallBmp();
	UnloadBitmap(&goback);
}
static int UserSetProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam) {
	HDC hdc;
	switch (message) {
	case MSG_CREATE:
		LoadUserSetBmp();
...
static void USSNotif(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		switch (id) {
		case IDC_ST_VIGB:
...
void AddUSControls(HWND main) {
	...
	SetNotificationCallback(sgbHwnd, USSNotif);
}
int WinUserSet(HWND hosting) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = UserSetProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
