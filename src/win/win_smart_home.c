...
BITMAP goback;
void LoadSmartHomeBmp() {
	LoadBgBmp();
	LoadWallBmp();
	LoadBitmap(HDC_SCREEN, &goback, op_bmp_name[0]);
}
void UnloadSmartHomeBmp() {
	UnloadBgBmp();
	UnloadWallBmp();
	UnloadBitmap(&goback);
}
static int SmartHomeProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam) {
	HDC hdc;
	switch (message) {
	case MSG_CREATE:
		LoadSmartHomeBmp();
...
int WinSmartHome(HWND hosting) {
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = SmartHomeProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
